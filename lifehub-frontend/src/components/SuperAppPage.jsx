...
import {
  ...
  CartDrawer,
  ProductDetailPage,
} from "./MarketplaceComponents.jsx";
import {
  StoryBar,
  AddContactModal,
  TypingIndicator,
...
function tabSubtitle(tabId) {
  ...
  if (key === "chat")
    return "Direct and group conversations with live delivery state";
  if (key === "marketplace")
    return "Nearby groceries with pricing and reliability scoring";
  if (key === "services")
    return "Book and track service providers around your location";
  if (key === "orders")
    return "Order lifecycle, delivery OTP, and feedback pipeline";
  if (key === "seller")
    return "Inventory management, product uploads, and quantity control";
  if (key === "wallet")
    return "Topups, settlements, and transaction visibility";
  if (key === "ops") return "Workflow and reconciliation operations";
  if (key === "profile")
    return "Account settings, security, and notification preferences";
  return "Module";
}
...
export default function SuperAppPage({
  ...
  const canCancelServiceRequest =
    hasRole("CUSTOMER") || hasRole("PROVIDER") || hasRole("ADMIN");
  const canCompleteServiceRequest =
    hasRole("CUSTOMER") || hasRole("PROVIDER") || hasRole("ADMIN");
  ...
  const tabs = useMemo(() => {
    ...
    if (canAccess.marketplace)
      available.push({ id: "marketplace", label: "Marketplace" });
    if (canAccess.services)
      available.push({ id: "services", label: "Book Service" });
  ...
  const [serviceRequestsModalOpen, setServiceRequestsModalOpen] =
    useState(false);
  ...
  const [pendingOrderAfterPayment, setPendingOrderAfterPayment] =
    useState(false);
  ...
  const marketplaceCategories = useMemo(() => {
    ...
    return [
      "all",
      ...Array.from(set).filter((value) => value !== "uncategorized"),
      "uncategorized",
    ];
  ...
  const visibleShopProducts = useMemo(() => {
    ...
    return shopProducts.filter(
      (product) => categoryKey(product?.category) === marketCategory,
    );
  ...
  const visibleProductResults = useMemo(() => {
    ...
    return productResults.filter(
      (result) => categoryKey(result?.category) === marketCategory,
    );
  ...
  const visibleRecommendedProducts = useMemo(() => {
    ...
    return recommendedProducts.filter(
      (item) => categoryKey(item?.category) === marketCategory,
    );
  ...
  const relatedMarketplaceProducts = useMemo(() => {
    ...
    const pushIfRelated = (candidate) => {
      ...
      if (
        ...
        categoryKey(selectedMarketplaceDetail.category)
      )
        return;
      seen.add(key);
  ...
  const moduleGroups = useMemo(() => {
    ...
    const groupDefs = [
      { id: "core", label: "Core", items: ["home"] },
      {
        id: "commerce",
        label: "Commerce",
        items: ["marketplace", "orders", "wallet"],
      },
  ...
  const filteredConversations = useMemo(() => {
    ...
    return conversations.filter((conv) =>
      [conv.id, conv?.peers?.[0]?.name, conv?.peers?.[0]?.phone]
        .filter(Boolean)
        .some((value) =>
          String(value)
            .toLowerCase()
            .includes(deferredChatSearch.trim().toLowerCase()),
        ),
    );
  ...
  const dashboardModuleUsage = useMemo(
    () => [
      ...
      {
        label: "Messages",
        value: totalUnreadChats + conversations.length,
        tone: "chat",
      },
  ...
  const activeTabMeta = useMemo(() => {
    ...
    if (activeTab === "chat") {
      return {
        ...
        description:
          "Stay connected with shops, providers, and support in one place.",
      };
    }
    if (activeTab === "marketplace") {
      return {
        ...
        eyebrow: "Commerce",
        description:
          "Discover shops and products near you with trusted ratings.",
      };
    }
    ...
    if (activeTab === "ops") {
      return {
        ...
        eyebrow: "Admin",
        description:
          "System health, SLA monitoring, and event pipeline status.",
      };
    }
    return {
  ...
  const visibleConversations = useMemo(() => {
    if (chatListMode === "unread") {
      return filteredConversations.filter(
        (conv) => Number(conv.unreadCount || 0) > 0,
      );
    }
    if (chatListMode === "groups") {
      return filteredConversations.filter(
        (conv) => String(conv.type || "").toUpperCase() === "GROUP",
      );
    }
  ...
  const marketplaceSavings = Number(
    cart
      .reduce(
        (sum, item) =>
          sum + Number(item.price || 0) * 0.12 * Number(item.quantity || 0),
        0,
      )
      .toFixed(2),
  );
  ...
  const activeTypingLabel = useMemo(() => {
    ...
    const row = Object.values(typingUsers).find((entry) => {
      ...
      if (String(entry.conversationId) !== String(selectedConversationId))
        return false;
    ...
    try {
      ...
      setMessages((prev) => {
        if (mode === "prepend") {
          ...
          return [
            ...ordered.filter((item) => !seen.has(String(item.id))),
            ...prev,
          ];
        }
        return ordered;
  ...
  async function sendMessage() {
    try {
      ...
      if (!selectedConversationId || (!text && !pendingAttachments.length))
        return;
  ...
  function removePendingAttachment(fileId) {
    setPendingAttachments((prev) =>
      prev.filter((item) => String(item.fileId) !== String(fileId)),
    );
  }
  ...
  async function uploadChatAttachment(file, options = {}) {
    const isPrivate =
      options.isPrivate !== undefined ? Boolean(options.isPrivate) : true;
    ...
    if (
      String(init.provider || "").toLowerCase() === "local" &&
      init.storagePath
    ) {
      const fallbackRoot = resolveApiBaseUrl();
      if (fallbackRoot) {
        uploadTargets.push(
          `${fallbackRoot}/upload/${String(init.storagePath).replace(/^\/+/, "")}`,
        );
      }
    }
    ...
    if (!uploadSucceeded) {
      ...
      if (message.toLowerCase().includes("failed to fetch")) {
        throw new Error(
          "Attachment upload failed: network/CORS issue. Check backend MEDIA_PUBLIC_BASE_URL and CORS_ORIGINS.",
        );
      }
  ...
  async function sendContactRequest(phoneOverride = "") {
    try {
      ...
      if (payload?.conversationId) {
        setToast(
          `Contact connected. Chat with ${payload?.contact?.name || payload?.contact?.phone}.`,
        );
        await openConversation(payload.conversationId);
      } else {
        setToast(
          `Request sent to ${payload?.contact?.name || payload?.contact?.phone}.`,
        );
      }
    } catch (err) {
  ...
  async function respondToContactRequest(requestId, action) {
    try {
      const payload = await api(
        `/chat/contacts/requests/${requestId}/respond`,
        {
          method: "POST",
      ...
      await Promise.all([loadContactDirectory(), loadConversations()]);
      if (
        String(action || "").toUpperCase() === "ACCEPT" &&
        payload?.conversationId
      ) {
        setToast(
          `${payload?.contact?.name || "Contact"} added to your chat list.`,
        );
        await openConversation(payload.conversationId);
      } else {
  ...
  async function pickContactsFromDevice() {
    if (!navigator.contacts?.select) {
      setError(
        "Device contact API is not supported in this browser. Use manual phone numbers.",
      );
      return;
    }
    try {
  ...
  async function acceptIncomingCall() {
    try {
      if (
        !callSession.roomId ||
        !callSession.offerSdp ||
        !callSession.incomingFrom
      )
        return;
      cleanupPeerConnection();
      cleanupMediaStreams();
      ...
      await pc.setRemoteDescription(
        new RTCSessionDescription(callSession.offerSdp),
      );
  ...
  async function searchShops(override = null) {
    ...
    if (canAccess.seller) {
      const ownedShop = shopRows.find(
        (shop) => String(shop.ownerUserId || "") === String(user.id || ""),
      );
      if (
        ownedShop &&
        String(sellerForm.shopId || "") !== String(ownedShop.id)
      ) {
  ...
  async function submitReview({ rating, comment, imageFile }) {
    ...
    try {
      ...
      if (imageFile) {
        ...
        const uploaded = await uploadChatAttachment(imageFile, {
          isPrivate: false,
        });
      ...
      await api(
        `/marketplace/shops/${selectedMarketplaceProduct.shopId}/feedback`,
        {
          method: "POST",
          body: JSON.stringify({
            rating,
  ...
  async function openMarketplaceProduct(item, fallbackShop = null) {
    const normalized = normalizeMarketplaceProduct(
      item,
      fallbackShop || selectedShop,
    );
    if (!normalized) return;
    try {
